...
bool DaemonManager::start(const QString &flags, bool testnet, const QString &dataDir)
{
    ...
    qDebug() << "starting nxboostd " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM nxboostd.exe");
#else
                QProcess::execute("pkill nxboostd");
#endif
            }
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/nxboostd.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/nxboostd";
#endif
